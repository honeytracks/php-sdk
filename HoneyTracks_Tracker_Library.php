 ...
 */
class HoneyTracks_Tracker_Library {
		...
		if(is_array($DefaultData)) {
			foreach(array('ApiKey', 'SecretKey') AS $varname) {
				if(!key_exists($varname, $DefaultData))
					throw new HoneyTracks_Tracker_Library_Exception(1, $varname);
	 ...
	 * 
	 * @param array $config
	 * @return HoneyTracks_Tracker_Library
	 * @throws HoneyTracks_Tracker_Library_Exception
	 */
	public static function Factory($DefaultData = null, array $ConfigurationData = null) {
		if(is_null(self::$SingletonInstance)) {
			if(is_null($DefaultData))
				throw new HoneyTracks_Tracker_Library_Exception(2);
	...
	public function setOptions($Options) {
		if(!is_array($Options)) throw new HoneyTracks_Tracker_Library_Exception(4);
	...
	public function setConfigurationByArray($ConfigurationData) {
		foreach($ConfigurationData AS $k => $v) {
			if(!$this->setConfiguration($k, $v))
				throw new HoneyTracks_Tracker_Library_Exception(6, $k);
		}
	}
	...
	public function Track($Action, $Data = array()) {
		if(!is_array($Data))
			throw new HoneyTracks_Tracker_Library_Exception(3);
	...
	protected function AddHTTPTrackingCall($data) {
		foreach($this->RequiredValues AS $key) {
			if(
				...
				throw new HoneyTracks_Tracker_Library_Exception(5, $key, print_r($data, true));
		}
	...
	protected function ExecuteHTTPQueue() {
		if(is_array($this->__HTTP_POST_QUEUE) && sizeof($this->__HTTP_POST_QUEUE) > 0) {
			...
			do {
				...
				} catch(HoneyTracks_Tracker_Library_Transport_Exception $Exception) {
	...
	protected function HTTPPost($url, $data) {
		...
		foreach($data AS $varName => $varValue) {
			if(is_array($varValue)) {
				...
				$postContent[] = urlencode($varName).'='.urlencode($subVarValue);
		}
		...
		if($this->Configuration['SEND_IMMEDIATELY'] !== true && !is_null($this->Configuration['PACKETS_STORAGE_PATH'])) {
			...
			if($this->Configuration['USE_CURL'] !== true) {
				...
				if(!$fp) {
					...
					throw new HoneyTracks_Tracker_Library_Transport_Exception(1, $url, $errorMessage['message']);
				}
				$response = @stream_get_contents($fp);
				@fclose($fp);
				if($response === false) {
					$errorMessage = error_get_last();
					throw new HoneyTracks_Tracker_Library_Transport_Exception(2, $url, $errorMessage['message']);
				}
			} else {
				$ch = curl_init($url);
				...
				if(!$response) {
					$error = curl_error($ch);
					@curl_close($ch);
					throw new HoneyTracks_Tracker_Library_Transport_Exception(2, $url, $error);
				}
				curl_close($ch);
			}
			return $response;
...
class HoneyTracks_Tracker_Library_Exception extends Exception {
    ...
    public function __construct($code) {
        ...
        if(!key_exists($code, $this->EXCEPTIONS)) $message = HoneyTracks_Tracker_Library_Exception::NO_VALID_EXCEPTION_CODE;
        else $message = $this->EXCEPTIONS[$code];

        if($argc > 1 || $message == HoneyTracks_Tracker_Library_Exception::NO_VALID_EXCEPTION_CODE) {
...
class HoneyTracks_Tracker_Library_Transport_Exception extends HoneyTracks_Tracker_Library_Exception {
